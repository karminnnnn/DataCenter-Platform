...
public class DataSourceServiceImpl extends BaseServiceImpl<DataSourceDao, DataSourceEntity> implements DataSourceService {
	...
	public PageResult<DataSourceVO> page(DataSourceQuery query) {
		...
		return new PageResult<>(DataSourceConvert.INSTANCE.convertList(page.getRecords()), page.getTotal());
	}

	private LambdaQueryWrapper<DataSourceEntity> getWrapper(DataSourceQuery query) {
		LambdaQueryWrapper<DataSourceEntity> wrapper = Wrappers.lambdaQuery();
		wrapper.like(StrUtil.isNotBlank(query.getName()), DataSourceEntity::getName, query.getName());
		wrapper.like(StrUtil.isNotBlank(query.getDatabaseName()), DataSourceEntity::getDatabaseName, query.getDatabaseName());
		wrapper.like(StrUtil.isNotBlank(query.getDatabaseSchema()), DataSourceEntity::getDatabaseSchema, query.getDatabaseSchema());
	...
	private void setJdbcUrlByEntity(DataSourceEntity entity) {
	...
	public void delete(List<Long> idList) {
		...
		// 确实是根据id来删除data_datasource中的数据
		removeByIds(idList);
	}

	@Override
	public void testOnline(DataSourceVO vo) {
	...
	private List<TableVo> getTables(DataSourceEntity DataSourceEntity) {
		...
		List<TableDescription> tableDescriptions = metaDataService.queryTableList(StringUtil.isBlank(DataSourceEntity.getJdbcUrl()) ? productTypeEnum.getUrl()
						.replace("{host}", DataSourceEntity.getDatabaseIp())
						.replace("{port}", DataSourceEntity.getDatabasePort())
						.replace("{database}", DataSourceEntity.getDatabaseName()) : DataSourceEntity.getJdbcUrl(), DataSourceEntity.getUserName(), DataSourceEntity.getPassword(),
				DataSourceEntity.getDatabaseSchema());
		if (DataSourceEntity.getId() == null) {
			//中台库返回ods层的表
			return BeanUtil.copyListProperties(tableDescriptions, TableVo::new).stream().filter(item -> item.getTableName().startsWith(DataHouseLayer.ODS.getTablePrefix())).collect(Collectors.toList());
		}
	...
	public List<DataSourceVO> listAll() {
		LambdaQueryWrapper<DataSourceEntity> wrapper = Wrappers.lambdaQuery();
		dataScopeWithOrgId(wrapper);
		return DataSourceConvert.INSTANCE.convertList(baseMapper.selectList(wrapper));
	}
	...
	private SqlGenerationVo getSqlGenerationVo(String tableName, String tableRemarks, DataSourceEntity entity) {
		setJdbcUrlByEntity(entity);
		ProductTypeEnum productTypeEnum = ProductTypeEnum.getByIndex(entity.getDatabaseType());
		IMetaDataByJdbcService service = new MetaDataByJdbcServiceImpl(productTypeEnum);
	...
	public List<ColumnDescriptionVo> middleDbClumnInfo(String tableName) {
		DataSourceEntity entity = buildMiddleEntity();
	 ...
	 * @return
	 */
	private DataSourceEntity buildMiddleEntity() {
		DataSourceEntity entity = new DataSourceEntity();
		DataProjectCacheBean project = getProject();
