<template>
	<el-dialog v-model="visible" title="项目成员" :close-on-click-modal="false">
		<el-form :inline="true" :model="state.queryForm" @keyup.enter="getDataList()">
			...
			<el-form-item>
				<el-button type="danger" @click="delProjectBatchHandle()">删除</el-button>
			</el-form-item>
		</el-form>
...
const state: IHooksOptions = reactive({
	dataListUrl: '/sys/user/page-project',
	deleteUrl: '/sys/user/project',
	queryForm: {
		username: '',
		mobile: '',
		gender: '',
...
const init = (id?: number) => {
	visible.value = true
	state.queryForm.projectId = id
	state.deleteUrl = '/sys/user/project/' + id
	getDataList()
}
