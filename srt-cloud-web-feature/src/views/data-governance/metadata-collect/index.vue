<template>
	<el-card body-style="height: calc(100vh - 170px )">
		<el-form :inline="true" :model="state.queryForm" @keyup.enter="getDataList()">
			...
			<el-form-item>
				<fast-select
					v-model="state.queryForm.strategy"
					dict-type="metadata_collect_strategy"
					placeholder="入库策略"
					clearable
					filterable
				></fast-select>
			...
			<el-table-column label="操作" fixed="right" header-align="center" align="center" width="330">
				<template #default="scope">
					...
					<el-button
						...
						v-auth="'data-governance:metadata-collect:release'"
						type="primary"
						link
						@click="release(scope.row.id)"
						>发布</el-button
					>
					<el-button v-if="scope.row.status == 1" v-auth="'data-governance:metadata-collect:cancel'" type="primary" link @click="cancel(scope.row.id)"
						>取消发布</el-button
					>
					<el-button
						v-if="scope.row.status == 0"
						v-auth="'data-governance:metadata-collect:update'"
						type="primary"
						link
					...
					<el-button
						v-if="scope.row.status == 0"
						v-auth="'data-governance:metadata-collect:delete'"
						type="primary"
						link
