			...
			<el-tabs tab-position="top">
				<el-tab-pane label="请求测试">
					<el-form ref="apiTestFormRef" :rules="apiTestFormRules" :model="apiTestForm">
						<div v-if="previlege == 1"><b>token 申请：</b></div>
						<br />
						<el-form-item v-if="previlege == 1" prop="tokenUrl" label-width="auto">
							<el-input v-model="apiTestForm.tokenUrl"></el-input>
						</el-form-item>
						<div v-if="previlege == 1"><el-button type="primary" @click="requestToken()">获取token</el-button><br /><br /></div>
						<div><b>请求地址：</b></div>
						<br />
...
const clickRequest = () => {
	...
	if (apiTestForm.type == 'GET') {
		...
		requestClient
			.get(apiTestForm.url + requestStr, config)
			.then(res => {
		...
		requestClient
			.post(apiTestForm.url, jsonParam, config)
			.then(res => {
				responseTestResultRef.value.setEditorValueAndFormat(JSON.stringify(res.data))
			})
			.catch(err => {
		...
		requestClient
			.put(apiTestForm.url, jsonParam, config)
			.then(res => {
				responseTestResultRef.value.setEditorValueAndFormat(JSON.stringify(res.data))
			})
			.catch(err => {
