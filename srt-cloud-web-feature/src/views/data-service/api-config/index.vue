	...
	<el-card v-show="ifShow">
		<div :class="ifAuth ? 'apiConfigAuthDivClass' : 'apiConfigDivClass'">
			<el-form :inline="true" :model="state.queryForm" @keyup.enter="getDataList()">
				...
				<el-form-item v-if="!ifMarket">
					<el-select v-model="state.queryForm.databaseId" clearable filterable placeholder="数据库">
						<el-option v-for="(item, index) in state.databaseList" :key="item.id" :label="`[${item.id}]${item.name}`" :value="item.id"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item v-if="!ifMarket">
				...
				<el-form-item>
					<el-button @click="getDataList()" v-if="ifAuth" type="primary">查询</el-button>
					<el-button @click="getDataList()" v-else>查询</el-button>
				</el-form-item>
				<el-form-item>
					<el-button v-auth="'data-service:api-config:save'" type="primary" @click="addOrUpdateHandle()" v-if="!ifAuth">新增</el-button>
				</el-form-item>
			...
			>
				...
				<el-table-column show-overflow-tooltip label="分组" header-align="center" align="center" width="150" v-if="!ifMarket">{{
					state.path
				}}</el-table-column>
				<el-table-column
					prop="groupPath"
				...
				<el-table-column label="操作" fixed="right" header-align="center" align="center" width="260">
					<template #default="scope">
						...
						<el-button
							v-auth="'data-service:app:auth'"
							type="primary"
							v-show="ifAuth && scope.row.authId == null && !ifMarket"
							link
						...
						<el-button
							...
							v-show="ifAuth && scope.row.authId != null && !ifMarket"
							link
							@click="changeAuth(scope.row.authId)"
							>修改授权</el-button
						>
						<el-button
							v-auth="'data-service:app:cancel-auth'"
							type="primary"
							v-show="ifAuth && scope.row.authId != null && !ifMarket"
							link
							@click="cancelAuth(scope.row.authId)"
							>取消授权</el-button
						>
						<el-button
							v-auth="'data-service:api-config:online'"
							type="primary"
							v-show="scope.row.status == 0 && !ifAuth"
							link
						...
						<el-button
							v-auth="'data-service:api-config:offline'"
							type="primary"
							v-show="scope.row.status == 1 && !ifAuth"
							link
							@click="offline(scope.row.id)"
							>下线</el-button
						>
						<el-button v-auth="'data-service:api-config:update'" type="primary" v-show="!ifAuth" link @click="addOrUpdateHandle(scope.row.id)"
							>修改</el-button
						>
						<el-button
							v-auth="'data-service:api-config:delete'"
							type="primary"
							v-show="scope.row.status == 0 && !ifAuth"
							link
...
const ifMarket = ref(false)
/* 初始化文件分组id */
const init = (groupId: number, path: any, appIdVal: any, ifAuthVal: any, ifMarketVal: any) => {
	...
	ifMarket.value = ifMarketVal ? ifMarketVal : false
	if (ifMarket.value) {
		state.queryForm.ifMarket = 1
	}
	if (ifAuth.value) {
